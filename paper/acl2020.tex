...
\title{Exploring Methods for Historical Text Normalization}
...
\maketitle
\begin{abstract}
This work aims to explore some different methods available for normalization and lemmatisation of historical texts: substitution-based methods (Norma) and character-based statistical machine translation (cSMTiser). The material used in this endeavour are taken from the MATHiR resource, containing texts in Old Swedish (ca 1225-1526).
\end{abstract}

\section{Introduction}
Historical texts that were written before any spelling standardization had taken place in their language of medium tend to contain a lot of spelling variation. For example... This becomes problematic when you want to parse them for further processing or research them in a corpus. There exists several different methods for normalizing such text, i.e. map them to a standard form. 
...
Rule-based methods instead try to pick up on regularities in the variations, e.g. that the letter \emph{v} is often written like \emph{u}. // Write something more about this //

The distance-based methods instead look at the number of edits needed to go from a variant spelling to a standard form, often using the Levenshtein distance. Adesam et al. (2012) use the Levensthein distance to derive substitution rules for Old Swedish, which perform better when compared to manually compiled substitution rules from dictionaries.
